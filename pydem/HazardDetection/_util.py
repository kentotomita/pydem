...
@jit
def footprint_checker_float(lander_type: str, xi_arr: np.ndarray, yi_arr: np.ndarray, rmpp: float, N_RLANDER: int):
    """Return the list of relative locations (pix) of the lander footprint
    ...
    for ti in range(n):
        ...
        for i in range(-N_RLANDER, N_RLANDER + 1):
            for j in range(-N_RLANDER, N_RLANDER + 1):
                xp = i * rmpp
                yp = j * rmpp
                ...
                if lander_type == "triangle":
                    within_footprint[ti, i, j] = (cond1 and cond2 and cond5)
                else:
                    within_footprint[ti, i, j] = (cond1 and cond2 and cond3 and cond4)
